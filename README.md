...
**Latest release: 1.22.0** — [TNT-Setup-1.22.0.exe](https://github.com/rhuntertec/TNT/releases/download/v1.22.0/TNT-Setup-1.22.0.exe) · [release notes](https://github.com/rhuntertec/TNT/releases/latest) · [checksum](https://github.com/rhuntertec/TNT/releases/download/v1.22.0/TNT-Setup-1.22.0.exe.sha256)
...
| **Faults** | What is wrong with this network, found without asking it anything. It starts with the service and watches three things no other tool looks at: every adapter’s error and discard counters, their live address configuration, and the ARP table over time. Frames arriving damaged is cabling, a connector or a duplex mismatch; frames this PC could not send because its queue was full is congestion — they are never added together, because they send you to opposite ends of the problem. One address answered by two MAC addresses on the same network is the duplicate IP that makes a device work intermittently for no visible reason. Errors and discards are judged on the last five minutes, so a fault is caught however long TNT has been running and a fix shows within minutes; what has happened since TNT started watching is shown beside it, never the adapter’s lifetime total alone. It says “clean” only when it could read everything it checks. Nothing is sent and no administrator rights are needed. |
